...
import { Plus, Edit2, Trash2, ChevronDown, ChevronRight, AlertTriangle, Loader2, Save, X } from 'lucide-react';
...
export function CategoriesView({ taxonomy, transactions, onUpdate, onCategorySelect }: CategoriesViewProps) {
  ...
  const [editingSubcategory, setEditingSubcategory] = useState<{ cat: string, old: string, new: string } | null>(null);
  ...
  const handleDeleteSubcategory = async (cat: string, subcat: string) => {
    ...
    try {
      ...
      if (inUse) {
        setError(`Cannot delete "${subcat}" under "${cat}" because it is assigned to existing transactions.`);
        return;
      }
      const nextTax = { ...taxonomy };
  ...
  const saveCategoryEdit = async () => {
    ...
    if (old === newName) { setEditingCategory(null); return; }
    if (taxonomy[newName]) { setError("Category already exists."); return; }
  ...
  const saveSubcategoryEdit = async () => {
    ...
    if (old === newName) { setEditingSubcategory(null); return; }
    if (taxonomy[cat].includes(newName)) { setError("Subcategory already exists."); return; }
  ...
  const addNewCategory = async () => {
    ...
    if (!val) return;
    if (taxonomy[val]) { setError("Category already exists."); return; }
  ...
  const addNewSubcategory = async (cat: string) => {
    ...
    if (taxonomy[cat].includes(val)) { setError("Subcategory already exists."); return; }
    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Manage the categories and subcategories available for transactions.</p>
        </div>
        {isEmpty && (
          <button onClick={handleInit} disabled={loading} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium flex items-center gap-2">
      ...
      {!isEmpty && (
        <div className="space-y-4">
          <div className="flex gap-2">
            ...
            />
            <button onClick={addNewCategory} disabled={!newCategory.trim() || loading} className="px-4 py-2 bg-slate-900 text-white rounded-lg hover:bg-slate-800 disabled:opacity-50">
              Add Category
            </button>
          </div>

          <div className="divide-y divide-slate-100 border border-slate-200 rounded-xl overflow-hidden">
            {Object.keys(taxonomy).sort().map(cat => {
              ...
              return (
                <div key={cat} className="bg-white">
                  <div className="flex items-center gap-3 p-4 hover:bg-slate-50 group">
                    <button onClick={() => toggleExpand(cat)} className="p-1 hover:bg-slate-200 rounded text-slate-400">
                      {isExpanded ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                    </button>
                    
                    {isEditingCat ? (
                      <div className="flex-1 flex gap-2">
                        <input autoFocus value={editingCategory.new} onChange={e => setEditingCategory({ ...editingCategory, new: e.target.value })} onKeyDown={e => e.key === 'Enter' && saveCategoryEdit()} className="px-2 py-1 border rounded text-sm flex-1" />
                        <button onClick={saveCategoryEdit} className="text-green-600 p-1"><Save className="w-4 h-4" /></button>
                    ...
                    {!isEditingCat && (
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => setEditingCategory({ old: cat, new: cat })} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded" title="Edit">
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleDeleteCategory(cat)} className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded" title="Delete">
                    ...
                    <div className="bg-slate-50 pl-12 pr-4 py-3 border-t border-slate-100">
                      <ul className="space-y-1">
                        ...
                        {taxonomy[cat].map(subcat => {
                          ...
                          return (
                            <li key={subcat} className="flex items-center gap-2 py-1.5 px-2 hover:bg-slate-100 rounded group/sub">
                              {isEditingSub ? (
                                <div className="flex-1 flex gap-2">
                                  <input autoFocus value={editingSubcategory.new} onChange={e => setEditingSubcategory({ ...editingSubcategory, new: e.target.value })} onKeyDown={e => e.key === 'Enter' && saveSubcategoryEdit()} className="px-2 py-1 border rounded text-sm flex-1" />
                                  <button onClick={saveSubcategoryEdit} className="text-green-600 p-1"><Save className="w-4 h-4" /></button>
                                  <button onClick={() => setEditingSubcategory(null)} className="text-slate-400 p-1"><X className="w-4 h-4" /></button>
                                </div>
                              ) : (
                                <>
                                  <span 
                                  ...
                                  <div className="flex items-center gap-1 opacity-0 group-hover/sub:opacity-100 transition-opacity">
                                    ...
                                    <button onClick={() => handleDeleteSubcategory(cat, subcat)} className="p-1 text-slate-400 hover:text-red-600 rounded">
                        ...
                        {newSubcategory?.cat === cat ? (
                           <li className="flex items-center gap-2 py-1.5 px-2 mt-2">
                             <input autoFocus placeholder="Subcategory name..." value={newSubcategory.val} onChange={e => setNewSubcategory({ ...newSubcategory, val: e.target.value })} onKeyDown={e => e.key === 'Enter' && addNewSubcategory(cat)} className="px-2 py-1 border rounded text-sm flex-1" />
                             <button onClick={() => addNewSubcategory(cat)} className="text-green-600 p-1"><Save className="w-4 h-4" /></button>
                             <button onClick={() => setNewSubcategory(null)} className="text-slate-400 p-1"><X className="w-4 h-4" /></button>
                           </li>
                        ) : (
                          <li className="mt-2">
                            <button onClick={() => setNewSubcategory({ cat, val: "" })} className="text-sm text-blue-600 hover:text-blue-700 flex items-center gap-1 px-2 py-1">

...
async function analyze() {
  ...
  const txSnapshot = await db.collection('transactions').where('Account', '==', 'Checking').get();
  ...
  txSnapshot.forEach((doc) => {
    ...
    if (
      ...
      ((date.getMonth() === 6 && date.getDate() <= 5) ||
  ...
  transactions.forEach((t) => {
    console.log(
      `[${t.dateStr}] $${(t.Amount || 0).toFixed(2).padStart(8)} | ${t.Description?.substring(0, 40)}`
    );
  });
  ...
  recSnapshot.forEach((doc) => {
    ...
    if (data.active) {
      console.log(
        `[Expected Day: ${String(data.expectedDate).padStart(2)}] $${(data.averageAmount || 0).toFixed(2).padStart(8)} | ${data.merchantName} | ${data.frequency}`
      );
    }
  });
}

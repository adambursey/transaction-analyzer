...
interface ReviewQueueProps {
  ...
  onBulkApprove: (
    updates: { id: string; category: string; subcategory: string }[]
  ) => Promise<void>;
}

/**
 * ReviewQueue Component.
 * Displays transactions that are pending review (e.g., after an import with fuzzy matches).
 * Allows users to review, edit, approve, and bulk-approve transactions.
 *
 * @param props.transactions - The full list of transactions.
 * @param props.taxonomy - The current taxonomy mapping for dropdowns.
 * @param props.onApprove - Callback to approve a single transaction.
 * @param props.onBulkApprove - Callback to approve multiple transactions simultaneously.
 */
export function ReviewQueue({
  ...
  // Memoized array of transactions that applies both the text filter and the current sort configuration.
  ...
  const applyBulkEdit = async () => {
    ...
    setIsBulkApproving(true);
    try {
      // Map the selected IDs to update objects using the newly chosen category/subcategory
            ...
            {isBulkEditing ? (
              <div className="flex items-center gap-2">
                <select
                  ...
                  onChange={(e) => {
                    setBulkEditCategory(e.target.value);
                    setBulkEditSubcategory('');
                  }}
                ...
                <select
                  value={bulkEditSubcategory}
                  onChange={(e) => setBulkEditSubcategory(e.target.value)}
                  disabled={!bulkEditCategory}
                  className="bg-slate-800 text-white text-sm border border-slate-600 rounded px-2 py-1"
                >
                  <option value="">None</option>
                  {bulkEditCategory &&
                    taxonomy[bulkEditCategory]?.map((sub) => (
        ...
        <table className="w-full text-left border-collapse text-sm">
          <thead className="sticky top-0 bg-slate-900 shadow-sm z-10">
            <tr className="text-slate-400 border-b border-slate-700">
              ...
              >
                Subcategory{' '}
                {sortConfig?.key === 'Subcategory' && (sortConfig.direction === 'asc' ? '↑' : '↓')}
              </th>
          ...
          <tbody className="divide-y divide-slate-800">
            {filteredAndSorted.map((tx) => {
              ...
              return (
                <tr
                  key={tx.id}
                  className={`hover:bg-slate-800/30 transition-colors ${selectedIds.has(tx.id) ? 'bg-slate-800/20' : ''}`}
                >

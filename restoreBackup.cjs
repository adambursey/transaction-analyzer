...
async function restore() {
  ...
  for (const item of backupData) {
    ...
    if (item.data.Date && item.data.Date._seconds !== undefined) {
      item.data.Date = new Firestore.Timestamp(
        item.data.Date._seconds,
        item.data.Date._nanoseconds
      );
    }
    if (item.data.createdAt && item.data.createdAt._seconds !== undefined) {
      item.data.createdAt = new Firestore.Timestamp(
        item.data.createdAt._seconds,
        item.data.createdAt._nanoseconds
      );
    }
